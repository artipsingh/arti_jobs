    ...
    >
      <div>
        <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "4px" }}>
          <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 700, fontSize: "14px", color: "#e2e8f0" }}>{job.company}</div>
          <div style={{ fontSize: "11px", color: "#64748b" }}>—</div>
          <div style={{ fontSize: "12px", color: "#94a3b8" }}>{job.role}</div>
        </div>
        <div style={{ fontSize: "11px", color: "#475569" }}>{job.verdict}</div>
      </div>
      <div style={{ fontSize: "11px", color: "#475569", whiteSpace: "nowrap" }}>{job.salary}</div>
      <div style={{ whiteSpace: "nowrap" }}>
        <span className={`tag ${c.bg} ${c.text}`} style={{ border: `1px solid`, borderColor: c.border.replace("border-", "").replace("/40", "4D") }}>
